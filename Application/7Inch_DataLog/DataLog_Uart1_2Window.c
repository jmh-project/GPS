...
int  SmartLog_Uart1Two_KeyEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
...
int  SmartLog_Uart1Two_TimerEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
    ...
    if(gv.autoScreen == ON)
    {
        ...
        {
            gv.autoScreenTimer++;
            DataLog_Data_Display( USART_1, 2 );
        }
    }
    else {
        DataLog_Data_Display( USART_1, 2 );
...
int  SmartLog_Uart1Two_CreateEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
    ...
    TitleBar_MenuButton_Create(handle,comPort,SmartLog_Uart1Two_KeyEvent);
...
int  SmartLog_Uart1Two_PaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data) {
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
...
int  SmartLog_Uart1Two_CloseEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data) {
	//윈도우가 닫힐때 필요한처리를 한다.
	Windows *win=NULL;
...
int  SmartLog_Uart1Two_Window(HANDLE parent, HANDLE handle) {
	if(GetWindowName(" SmartLog_Uart1Two_Window")) 
	{
		Dprintf(" SmartLog_Uart1Two_Window:: Windows already Create\n");
		return 0;		
	}	
	makeWindow
    (
        LCD,								// Display Section Position, 이윈도우가 어디에 표시되는가?
        " SmartLog_Uart1Two_Window",			// Windows Name, 이이름으로 이윈도우의 핸들을 찾을수 있다. 64자
        ...
        SmartLog_Uart1Two_CreateEvent,		// CreateEvent
        SmartLog_Uart1Two_PaintEvent,			// PaintEvent
        NULL,	// MouseOnEvent
        NULL,	// MouseOffEvent
        NULL,							// MouseMoveEvent
        NULL,							// GestureEvent
        NULL,						// ClickEvent, 콜백함수를 등록한다.(콜백함수는 어떤것도 등록가능하다)
        SmartLog_Uart1Two_TimerEvent,			// TimerEvent
        ...
        SmartLog_Uart1Two_CloseEvent,			// Close Event, 윈도우가 닫힐때 발생한다.
