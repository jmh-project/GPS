/******************************************************************************
*       File name	: Windows_ SmartLog_Uart2Eight.c 
...
/******************************************************************************
							Example Program
******************************************************************************/
int  SmartLog_Uart2Eight_KeyEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
...
int  SmartLog_Uart2Eight_TimerEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
     ...
     if(gv.autoScreen == ON)
    {
        ...
        {
            gv.autoScreenTimer++;
            DataLog_Data_Display( USART_2, 8 );
        }
    }
    else
        DataLog_Data_Display( USART_2, 8 );
...
int  SmartLog_Uart2Eight_CreateEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
    ...
    char    pBuf[30];
    TitleBar_Create(handle,USART_2,pBuf);
    ...
    SButton(handle, 0, LCD_Y-65, 180, 64, "48G_Config_Icon2.bmp", "SENSOR",  PUSH_BUTTON, 1, SmartLog_Uart2Eight_KeyEvent);
    SButton(handle, 0+180, LCD_Y-65, 170, 64, "48G_TestBottle_Icon1.bmp", "CAL",  PUSH_BUTTON, 2, SmartLog_Uart2Eight_KeyEvent);
    SButton(handle, 0+180+170, LCD_Y-65, 170, 64, "48G_Network_Icon1.bmp", "SYSTEM",  PUSH_BUTTON, 3, SmartLog_Uart2Eight_KeyEvent);
    SButton(handle, 0+180+170+170, LCD_Y-65, 170, 64, "48G_USB_Icon1.bmp", "USB",  PUSH_BUTTON, 4, SmartLog_Uart2Eight_KeyEvent);
    if(gv.autoScreen == OFF)
        gv.handle[50]=SButton(handle, 0+180+170+170+170, LCD_Y-65, 170, 64, "48G_Display_Icon1.bmp", "MANUAL",  PUSH_BUTTON, 5, SmartLog_Uart2Eight_KeyEvent);
    else
        gv.handle[50]=SButton(handle, 0+180+170+170+170, LCD_Y-65, 170, 64, "48G_Display_Icon1.bmp", "AUTO",  PUSH_BUTTON, 5, SmartLog_Uart2Eight_KeyEvent);
    SButton(handle, 0+180+170+170+170+170, LCD_Y-65, 164, 64, "48G_Display_Icon1.bmp", "NEXT",  PUSH_BUTTON, 6, SmartLog_Uart2Eight_KeyEvent);
...
int  SmartLog_Uart2Eight_PaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
...
int  SmartLog_Uart2Eight_CloseEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	//윈도우가 닫힐때 필요한처리를 한다.
	Windows *win=NULL;
...
{
	if(GetWindowName(" SmartLog_Uart2Eight_Window")) 
	{
		Dprintf(" SmartLog_Uart2Eight_Window:: Windows already Create\n");
		return 0;		
	}	
	makeWindow
    (
        LCD,								// Display Section Position, 이윈도우가 어디에 표시되는가?
        " SmartLog_Uart2Eight_Window",			// Windows Name, 이이름으로 이윈도우의 핸들을 찾을수 있다. 64자
        ...
        SmartLog_Uart2Eight_CreateEvent,		// CreateEvent
        SmartLog_Uart2Eight_PaintEvent,			// PaintEvent
        NULL,	// MouseOnEvent
        NULL,	// MouseOffEvent
        NULL,							// MouseMoveEvent
        NULL,							// GestureEvent
        NULL,						// ClickEvent, 콜백함수를 등록한다.(콜백함수는 어떤것도 등록가능하다)
        SmartLog_Uart2Eight_TimerEvent,			// TimerEvent
        ...
        SmartLog_Uart2Eight_CloseEvent,			// Close Event, 윈도우가 닫힐때 발생한다.
