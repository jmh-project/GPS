/******************************************************************************
*       File name	: Windows_ SmartLog_Uart4Two.c 
...
/******************************************************************************
							Example Program
******************************************************************************/
int  SmartLog_Uart4Two_KeyEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
...
int  SmartLog_Uart4Two_TimerEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
    ...
    if(gv.autoScreen == ON)
    {
        ...
        {
            gv.autoScreenTimer++;
            DataLog_Data_Display( USART_4, 2 );
        }
    }
    else
        DataLog_Data_Display( USART_4, 2 );
...
int  SmartLog_Uart4Two_CreateEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
...
#if defined(SEOUL_PROJECT)	
    NGroupPanel(handle, groupX, groupY, groupW, groupH, flash.Uart[comPort]->SetupName[0]);
	sprintf(data,"%s",gv.gpgamcAvalue);
    gv.handle[0] = NLabel(handle, textX, textY, textW, textH,gv.gpgamcAvalue,labelFontColor,labelBackColor,3);
    
    groupX = groupX + 10 + groupW;
    textX = groupX;
    NGroupPanel(handle, groupX, groupY, groupW, groupH, flash.Uart[comPort]->SetupName[1]);
    sprintf(data,"%s",gv.gpgamcNvalue);
    gv.handle[1] = NLabel(handle, textX, textY, textW, textH,gv.gpgamcNvalue,labelFontColor,labelBackColor,3);
#endif    
    ...
    gv.titleBarTimeId = SLabel(handle, 1024-230, 5, 220, 40,pBuf,clBlack,RGB565CONVERT(230, 230, 230),1);
    SButton(handle, 0, LCD_Y-65, 180, 64, "48G_Config_Icon2.bmp", "SENSOR",  PUSH_BUTTON, 1, SmartLog_Uart4Two_KeyEvent);
    SButton(handle, 0+180, LCD_Y-65, 170, 64, "48G_TestBottle_Icon1.bmp", "CAL",  PUSH_BUTTON, 2, SmartLog_Uart4Two_KeyEvent);
    SButton(handle, 0+180+170, LCD_Y-65, 170, 64, "48G_Network_Icon1.bmp", "SYSTEM",  PUSH_BUTTON, 3, SmartLog_Uart4Two_KeyEvent);
    SButton(handle, 0+180+170+170, LCD_Y-65, 170, 64, "48G_USB_Icon1.bmp", "USB",  PUSH_BUTTON, 4, SmartLog_Uart4Two_KeyEvent);
	/*
	if(gv.autoScreen == OFF)
        gv.handle[50]=SButton(handle, 0+180+170+170+170, LCD_Y-65, 170, 64, "48G_Display_Icon1.bmp", "MANUAL",  PUSH_BUTTON, 5, SmartLog_Uart4Two_KeyEvent);
    else
        gv.handle[50]=SButton(handle, 0+180+170+170+170, LCD_Y-65, 170, 64, "48G_Display_Icon1.bmp", "AUTO",  PUSH_BUTTON, 5, SmartLog_Uart4Two_KeyEvent);
    */
	gv.handle[50]=SButton(handle, 0+180+170+170+170, LCD_Y-65, 170, 64, "48G_Shutdown_Icon1.bmp", "안전리셋",  PUSH_BUTTON, 5, SmartLog_Uart4Two_KeyEvent);	
	SButton(handle, 0+180+170+170+170+170, LCD_Y-65, 164, 64, "48G_Display_Icon1.bmp", "NEXT",  PUSH_BUTTON, 6, SmartLog_Uart4Two_KeyEvent);
...
int  SmartLog_Uart4Two_PaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
...
int  SmartLog_Uart4Two_CloseEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	//윈도우가 닫힐때 필요한처리를 한다.
	Windows *win=NULL;
...
{
	if(GetWindowName(" SmartLog_Uart4Two_Window")) 
	{
		Dprintf(" SmartLog_Uart4Two_Window:: Windows already Create\n");
		return 0;		
	}	
	makeWindow
    (
        LCD,								// Display Section Position, 이윈도우가 어디에 표시되는가?
        " SmartLog_Uart4Two_Window",			// Windows Name, 이이름으로 이윈도우의 핸들을 찾을수 있다. 64자
        ...
        SmartLog_Uart4Two_CreateEvent,		// CreateEvent
        SmartLog_Uart4Two_PaintEvent,			// PaintEvent
        NULL,	// MouseOnEvent
        NULL,	// MouseOffEvent
        NULL,							// MouseMoveEvent
        NULL,							// GestureEvent
        NULL,						// ClickEvent, 콜백함수를 등록한다.(콜백함수는 어떤것도 등록가능하다)
        SmartLog_Uart4Two_TimerEvent,			// TimerEvent
        ...
        SmartLog_Uart4Two_CloseEvent,			// Close Event, 윈도우가 닫힐때 발생한다.
