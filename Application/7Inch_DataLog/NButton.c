...
void NButtonDraw(int  press, Windows *win, Graphics *Gr, int sx, int sy){
	int icon_size;
	// ColorCode
...
void NButtonDraw_New(int  press, Windows *win, Graphics *Gr, int sx, int sy){
	int icon_size;
	// ColorCode
	//	230, 134, 112, 112-----------------------------------134, 230
	//	136, 143,          -----------------------------------  143, 136,
...
int	NButtonMouseOnEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data) {
	int			sx=0, sy=0;
//	int			text_width=0;
	...
	if(win->Toggle==PUSH_BUTTON || (win->Toggle==TOGGLE_BUTTON && win->ToggleStatus==OFF))
	{
		...
		if(flash.systemColorNo!=6) {
			NButtonDraw_New(ON, win, Gr, sx, sy);
		}
		else {
			NButtonDraw(ON, win, Gr, sx, sy);
		}
	}
	else
	{
...
int	NButtonMouseOffEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data) {
	int			sx=0, sy=0;
//	int			text_width;
	...
	//!< 버튼의 외각을그린다.
	if(flash.systemColorNo!=6) {
		NButtonDraw_New(OFF, win, Gr, sx, sy);
	} 
	else {
		NButtonDraw(OFF, win, Gr, sx, sy);
	}
...
int	NButtonPaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data) {
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
...
//	int			text_width;
	...
	//!< 버튼의 외각을그린다.
	if(flash.systemColorNo!=6) {
		NButtonDraw_New(command, win, Gr, sx, sy); 
	} 
	else {
		NButtonDraw(command, win, Gr, sx, sy);
	}
...
//	PARAM	parameter;
    ...
    handle=makeWindow(
				...
				NULL,						// CreateEvent
				NButtonPaintEvent,			// PaintEvent
				NButtonMouseOnEvent,			// MouseOnEvent
				NButtonMouseOffEvent,		// MouseOffEvent
				NULL,						// MouseMoveEvent
				NULL,						// GestureEvent
