...
******************************************************************************/
//다이아몬드스타일의 버튼을 그린다.
void NDiamondPanelDraw(Windows *win, Graphics *Gr, int sx, int sy, int swidth, int sheight, int outline)
{
	int	width = swidth-1;
...
void NGroupPanelDraw(Windows *win, Graphics *Gr, int sx, int sy, int swidth, int sheight, int outline)
{
	int	width = swidth-1;
	int	height = sheight-1;
	// System Color로  사각영역을 그린다(채운다).
...
//좌우가 볼록한 패널을 그릴때 사용한다. 위젯용이다.
void NGroupPanelDraw_New(Windows *win, Graphics *Gr, int sx, int sy, int swidth, int sheight, int outline){
	int	width = swidth-1;
	int	height = sheight-1;
	// System Color로  사각영역을 그린다(채운다).
	//Gr->SetColor(RGB565CONVERT(191, 191, 191));
    ...
    int color = flash.systemGroupPanelColor;
    Gr->SetColor(color);
	Gr->FillRect(sx, sy, width, height);	
...
int	NGroupPanelPaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			sx=0, sy=0;
	...
	if(flash.systemColorNo==6) {
		...
		//타이틀 패널
		NDiamondPanelDraw(win, Gr, sx, sy, win->width, TITLE_HEIGHT, 0);
		NGroupPanelDraw(win, Gr, sx, sy+TITLE_HEIGHT, win->width, win->height-TITLE_HEIGHT, 0);
		...
		NGroupPanelDraw_New(win, Gr, sx, sy, win->width, TITLE_HEIGHT, 1);
		SetFontAliasing("ng_eh_24.sif");
		int text_width = TextLenAliasing(win->Text, "ng_eh_24.sif");
		sx = 10 *2;
		sy = (TITLE_HEIGHT - 24) / 2;
		Gr->TextOutAliasing(sx, sy, win->Text, flash.systemFontColor, 0, "ng_eh_24.sif");
	}
...
U32 NGroupPanel(HANDLE ContainerHandle, int sx, int sy, int width, int height , char *text)
{
	...
	handle=makeWindow(
				...
				NULL,						// CreateEvent
				NGroupPanelPaintEvent,			// PaintEvent
				NULL,						// MouseOnEvent
				NULL,						// MouseOffEvent
				NULL,						// MouseMoveEvent
				NULL,						// GestureEvent
