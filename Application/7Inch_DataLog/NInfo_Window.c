...
******************************************************************************/
static char NInfo_Text[128];
/******************************************************************************
							Function Prototype
******************************************************************************/
		
U32 BIcon(HANDLE ContainerHandle, int sx, int sy, int width, int height , char *iconame, int backcolor);
static int	  NInfo_ClickEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
...
static int	  NInfo_CreateEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	...
	SLabel(handle, 20+60, 50+15, 590, 30, NInfo_Text, clBlack, RGB565CONVERT(191, 191, 191),  0);	 //
	//SLabel(handle, 20+60, 50+15+40, 500, 30, "", clBlack, RGB565CONVERT(191, 191, 191),  0);	

	//Close
	SButton(handle, 460, 180, 200, 54, "48G_LogOut_Icon1.bmp", "Close",  PUSH_BUTTON, 30, NInfo_ClickEvent);
...
static int	  NInfo_PaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle,"");;
...
{
	if(GetWindowName("NInfo_Window")) 
	{
		Dprintf("NInfo_Window:: Windows already Create\n");
		return 0;		
	}		
	int sx = (LCD_X - 690 ) / 2;
	int sy = (LCD_Y - 300 ) / 2;
	strcpy(NInfo_Text, text);
	makeWindow(
				LCD,
				"NInfo_Window",				// Windows Name
				...
				NInfo_CreateEvent,		// CreateEvent
				NInfo_PaintEvent,		// PaintEvent
				NULL,					// MouseOnEvent
				NULL,					// MouseOffEvent
				NULL,					// MouseMoveEvent
				NULL,					// GestureEvent
