...
void NTitleBarPanelDraw(Windows *win, Graphics *Gr, int sx, int sy, int outline)
{
...
void NTitleBarPanelDraw1(Windows *win, Graphics *Gr, int sx, int sy, int outline)
{
	int	width = win->width-1;
	int	height = win->height-1;
	// System Color로  사각영역을 그린다(채운다).
...
void NTitleBarPanelDraw2(Windows *win, Graphics *Gr, int sx, int sy, int outline)
{
	// ColorCode
    ...
    if(flash.systemStatus == ON)
    {
        IconDisplay(15, 1, "TitleLogo.bmp", RGB565CONVERT(191, 191, 191));	
    }
    else
    {
        IconDisplay(15, 1, "TitleLogError.bmp", RGB565CONVERT(191, 191, 191));	
    }
}

int NTitleBarTextDraw(Windows *win, Graphics *Gr)
{
	int	sx, sy;
	...
	if(strlen(win->Text))		
	{
        ...
        if( 0==strcmp(win->Text, "USART_1") )
        {
            
        }
        else if( 0==strcmp(win->Text, "USART_2") )
        {
            
        }
        else if( 0==strcmp(win->Text, "USART_3") )
        {
            
        }
        else if( 0==strcmp(win->Text, "USART_4") )
        {
            
        }
        else if( 0==strcmp(win->Text, "USART_5") )
        {
...
void NTitleBarDraw(int  press, Windows *win, Graphics *Gr, int sx, int sy)
{
	if(win->WidgetStyle==1)
		NTitleBarPanelDraw(win, Gr, sx, sy, 0);
	else if(win->WidgetStyle==2)
		NTitleBarPanelDraw1(win, Gr, sx, sy, 0); 
	else if(win->WidgetStyle==3)
		NTitleBarPanelDraw2(win, Gr, sx, sy, 0); 
	else if(win->WidgetStyle==4)
		OuterPanelDraw(win, Gr, sx, sy, 0); 
	
	NTitleBarTextDraw(win, Gr);
}

int	NTitleBarMouseOnEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
//	int			sx=0, sy=0;
	...
	//NTitleBarDraw(ON, win, Gr, sx, sy);
	Gr->Draw(parent, handle, command, param, Data);	
	return(1);
}

int	NTitleBarMouseOffEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle,"");
...
//	int sx=0, sy=0;
	...
	//if(win->Attr == READ_ONLY) return(0);	
 	//Dummy
	Gr->FillRect(0, 0, 0, 0);
	//NTitleBarDraw(OFF, win, Gr, sx, sy);
...
int	NTitleBarPaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			sx=0, sy=0;
...
U32 NTitleBar(HANDLE ContainerHandle, 
...
//	wid_struct.Attr = attr;	
	...
	handle=makeWindow(
				...
				NULL,						// CreateEvent
				NTitleBarPaintEvent,			// PaintEvent
				NULL,		// MouseOnEvent
				NULL,		// MouseOffEvent
				NULL,						// MouseMoveEvent
				NULL,						// GestureEvent
