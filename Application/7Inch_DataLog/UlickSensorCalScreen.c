...
int	 ULIKScreenEditButtonEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data) {
    ...
    if(win->ID==3) {//CODReal
        ...
        if(gv.ulikCalItem==0) {
            KeyPad(win->Parent, win->Handle,  gv.handle2[win->ID], (1024-345)/2, (LCD_Y-415)/2, 1, "", "CODreal");
        }
        else if(gv.ulikCalItem==1) {
            KeyPad(win->Parent, win->Handle,  gv.handle2[win->ID], (1024-345)/2, (LCD_Y-415)/2, 1, "", "TOCreal");
        }
            ...
            KeyPad(win->Parent, win->Handle,  gv.handle2[win->ID], (1024-345)/2, (LCD_Y-415)/2, 1, "", "COD_KHP");
        }
        else if(gv.ulikCalItem==4) {
            KeyPad(win->Parent, win->Handle,  gv.handle2[win->ID], (1024-345)/2, (LCD_Y-415)/2, 1, "", "TOCstd");
        }
    }
    else if(win->ID==4) {//COD_KHPsensor :: 자동적용
        //KeyPad(win->Parent, win->Handle,  gv.handle2[win->ID], (1024-345)/2, (LCD_Y-415)/2, 1, "", "COD_KHPsensor");
    }
    else if(win->ID==5) {//TURsensor :: 자동적용
        //KeyPad(win->Parent, win->Handle,  gv.handle2[win->ID], (1024-345)/2, (LCD_Y-415)/2, 1, "", "TURsensor");
    }

    //교정명령
    else if(win->ID==12) {//세척명령
        Dprintf("세정시작\n");
        Ulik_WindowCleaning(USART_1,flash.ulik.id,0x0F);
        SetWidgetStatus(gv.handle2[12],ON);
        ulikCleanCount=10;//sec
    }
    else if(win->ID==13) {//comfirm
        if(gv.ulikCalStep >= 5) {
            if(gv.ulikCalItem==0) {
            ...
            GetWidgetText(gv.handle2[3],pBuf);//COD  
            ...
            if(fdata > 0 && gv.ulikMessageStep==1) {    
                ...
                if(gv.ulikCalItem==0) {
                    ...
                    ULIK_MessageBox_Window("COD_KHP","교정 값을 적용합니다.");
                }
                else if(gv.ulikCalItem==4) {
                    ULIK_MessageBox_Window("TOCstd","교정 값을 적용합니다.");
                }
            } else {
                if(gv.ulikCalItem==0) {
                    ULIK_MessageBox_Window("COD","CODreal 값을 설정해야 합니다.");
                }
                else if(gv.ulikCalItem==1) {
                    ULIK_MessageBox_Window("TOC","TOCreal 값을 설정해야 합니다.");
                }
                else if(gv.ulikCalItem==2) {
                    ULIK_MessageBox_Window("TUR","TUR 값을 설정해야 합니다.");
                }
                else if(gv.ulikCalItem==3) {
                    ULIK_MessageBox_Window("COD_KHP","COD_KHP 값을 설정해야 합니다.");
                }
                else if(gv.ulikCalItem==4) {
                    ULIK_MessageBox_Window("TOCstd","TOCstd 값을 설정해야 합니다.");
                }
            }
        }
    }
    else if(win->ID==14) {//ClearLastCalib
        if(gv.ulikCalItem==0) {
            ULIK_MessageBox_Window("COD","마지막 교정 값을 초기화합니다.");        
        }
        else if(gv.ulikCalItem==1) {
            ULIK_MessageBox_Window("TOC","마지막 교정 값을 초기화합니다.");
        }
        else if(gv.ulikCalItem==2) {
            ULIK_MessageBox_Window("TUR","교정 값을 초기화합니다.");
        }
        else if(gv.ulikCalItem==3) {
            ULIK_MessageBox_Window("COD_KHP","교정 값을 초기화합니다.");
        }
        else if(gv.ulikCalItem==4) {
            ULIK_MessageBox_Window("TOCstd","교정 값을 초기화합니다.");
        }
    }
...
int	 ULIKScreenTimerEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data) {
    ...
    if(gv.ulikCalCmdSendNo==1) {            
        GetWidgetText(gv.handle2[3],pBuf);  
        float fdata = atof(pBuf);
        if(gv.ulikCalItem==0) {//COD  
            Ulik_CODReal(comPort, flash.ulik.id, fdata);
            gv.ulikCalCmdSendNo=4;//교정파라메터송신 다음단계
        }
        else if(gv.ulikCalItem==1) {//TOC
            Ulik_TOCReal(comPort, flash.ulik.id, fdata);
            gv.ulikCalCmdSendNo=4;//교정파라메터송신 다음단계
        }
        else if(gv.ulikCalItem==2) {//TUR
            Ulik_TUR_StdCalibration(comPort, flash.ulik.id, fdata);
            gv.ulikCalCmdSendNo=0;
            SetWidgetStatus(gv.handle2[13],OFF);
        }
        else if(gv.ulikCalItem==3) {//COD_KHP
            Ulik_COD_StdCalibration(comPort, flash.ulik.id, fdata);
            gv.ulikCalCmdSendNo=0;
            SetWidgetStatus(gv.handle2[13],OFF);
        }
        else if(gv.ulikCalItem==4) {//TOCstd
            Ulik_TOC_StdCalibration(comPort, flash.ulik.id, fdata);
            gv.ulikCalCmdSendNo=0;
            SetWidgetStatus(gv.handle2[13],OFF);
        }
    ...
    else if(gv.ulikCalCmdSendNo==2) {
        gv.ulikCalCmdSendNo=0;
        if(gv.ulikCalItem==0) {
            Ulik_CODcalibCtrl(comPort,flash.ulik.id,0x06,1);        
        }
        else if(gv.ulikCalItem==1) {
            Ulik_TOCcalibCtrl(comPort,flash.ulik.id,0x06,1);
        }
        else if(gv.ulikCalItem==2) {
            Ulik_TUR_CalibrationClear(comPort,flash.ulik.id);
        }
        else if(gv.ulikCalItem==3) {
            Ulik_COD_KPHCalibrationClear(comPort,flash.ulik.id);
        }
        else if(gv.ulikCalItem==4) {
            Ulik_TOC_StdCalibrationClear(comPort,flash.ulik.id);
        }
        Dprintf("교정(삭제)명령 송신완료\n");
    }
    else if(gv.ulikCalCmdSendNo==3) {
        gv.ulikCalCmdSendNo=0;
        if(gv.ulikCalItem==0) {
            Ulik_CODcalibCtrl(comPort,flash.ulik.id,0x06,2);        
        }
        else if(gv.ulikCalItem==1) {
            Ulik_TOCcalibCtrl(comPort,flash.ulik.id,0x06,2);
        }
        Dprintf("교정(삭제)명령 송신완료\n");
    }
    else if(gv.ulikCalCmdSendNo==4) {
        GetWidgetText(gv.handle2[4],pBuf);  //COD  
        float fdata = atof(pBuf);
        if(gv.ulikCalItem==0) {
            Ulik_COD_KHPsensor(comPort, flash.ulik.id, fdata);
        }
        else if(gv.ulikCalItem==1) {
            Ulik_TOCstd_sensor(comPort, flash.ulik.id, fdata);
        }
        Dprintf("교정(설정 %.3f)명령 송신완료\n",fdata);        
        gv.ulikCalCmdSendNo=5;//교정파라메터송신 다음단계
    }
    ...
    if(gv.ulikCalItem==0) {
        ...
        pBuf[10]=0;
        SetWidgetText(gv.handle2[1],pBuf);  //COD_KHP    
    ...
    if(gv.ulikCalItem==0) {
        ...
        SetWidgetText(gv.handle2[30],"COD_KHPsensor");
        SetWidgetText(gv.handle2[31],"TURsensor");
        SetWidgetText(gv.handle2[32],"CODreal");

        sprintf(pBuf,"%d-point calibration value", paraAddress+2);
        SetWidgetText(gv.handle2[22],pBuf);
        SetWidgetText(gv.handle2[33],"COD_KHPsensor");
        SetWidgetText(gv.handle2[34],"TURsensor");
        SetWidgetText(gv.handle2[35],"CODreal");
    } 
    else if(gv.ulikCalItem==1) {
        sprintf(pBuf,"%d-point calibration value", paraAddress+1);
        SetWidgetText(gv.handle2[21],pBuf);
        SetWidgetText(gv.handle2[30],"TOCstd_sensor");
        SetWidgetText(gv.handle2[31],"TURsensor");
        SetWidgetText(gv.handle2[32],"TOCreal");

        sprintf(pBuf,"%d-point calibration value", paraAddress+2);
        SetWidgetText(gv.handle2[22],pBuf);
        SetWidgetText(gv.handle2[33],"TOCstd_sensor");
        SetWidgetText(gv.handle2[34],"TURsensor");
        SetWidgetText(gv.handle2[35],"TOCreal");
    }
    
    if(gv.ulikCalItem==0 || gv.ulikCalItem==1) {
        //1-point calibration value    
        sprintf(pBuf,"%10.3f", gv.paraValue1[paraAddress]);    
        pBuf[10]=0;
        SetWidgetText(gv.handle2[6],pBuf);  //COD_KHPsensor   
        sprintf(pBuf,"%10.3f", gv.paraValue2[paraAddress]);
        pBuf[10]=0;
        SetWidgetText(gv.handle2[7],pBuf);  //TURsensor    
        sprintf(pBuf,"%10.3f", gv.paraValue3[paraAddress]);
        pBuf[10]=0;
        SetWidgetText(gv.handle2[8],pBuf);  //CODreal  
        
        //2-point calibration value
        paraAddress += 1;
        sprintf(pBuf,"%10.3f", gv.paraValue1[paraAddress]);    
        pBuf[10]=0;
        SetWidgetText(gv.handle2[9],pBuf);  //COD_KHPsensor   
        sprintf(pBuf,"%10.3f", gv.paraValue2[paraAddress]);
        pBuf[10]=0;
        SetWidgetText(gv.handle2[10],pBuf); //TURsensor    
        sprintf(pBuf,"%10.3f", gv.paraValue3[paraAddress]);
        pBuf[10]=0;
        SetWidgetText(gv.handle2[11],pBuf); //CODreal  
    } 
    else if(gv.ulikCalItem==2) {
        sprintf(pBuf,"%10.3f", gv.paraValue1[0]);    
        pBuf[10]=0;
        SetWidgetText(gv.handle2[6],pBuf);  //TURorg   
        sprintf(pBuf,"%10.3f", gv.paraValue2[0]);
        pBuf[10]=0;
        SetWidgetText(gv.handle2[7],pBuf);  //TURstd
    }
    else if(gv.ulikCalItem==3) {
        sprintf(pBuf,"%10.3f", gv.paraValue1[0]);    
        pBuf[10]=0;
        SetWidgetText(gv.handle2[6],pBuf);  //COD_KHPorg
        sprintf(pBuf,"%10.3f", gv.paraValue2[0]);
        pBuf[10]=0;
        SetWidgetText(gv.handle2[7],pBuf);  //COD_KHP
    }
    else if(gv.ulikCalItem==4) {
        sprintf(pBuf,"%10.3f", gv.paraValue1[0]);    
        pBuf[10]=0;
        SetWidgetText(gv.handle2[6],pBuf);  //TOCstd_org
        sprintf(pBuf,"%10.3f", gv.paraValue2[0]);
        pBuf[10]=0;
        SetWidgetText(gv.handle2[7],pBuf);  //TOCstd
    }
...
int	 ULIKScreenCreateEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data) {
    ...
    if(gv.ulikCalItem==0) {
        ...
        Label(handle, sx, sy+height+side, width, height, "COD_KHP",  clBlack);
        gv.handle2[1]=SButton(handle, sx+width+side, sy+height+side, width, height, 0, pBuf,  NULL, 1, NULL); 
        
        Label(handle, sx, sy+height+side+height+side, width, height, "TUR",  clBlack);
        ...
        Label(handle, sx, sy+height+side+height+side+height+side+height+side, width, height, "CODreal",  clBlack);
        ...
        Label(handle, sx, sy+height+side+height+side+height+side+height+side+height+side, width, height, "COD_KHPsensor",  clBlack);
        gv.handle2[4]=SButton(handle, sx+width+side, sy+height+side+height+side+height+side+height+side+height+side, width, height, 0, pBuf,  SELECT_BUTTON, 4, ULIKScreenEditButtonEvent); 
        
        Label(handle, sx, sy+height+side+height+side+height+side+height+side+height+side+height+side, width, height, "TURsensor",  clBlack);
        ...
        gv.handle2[30]=Label(handle, sx, sy, width, height, "COD_KHPsensor",  clBlack);
        gv.handle2[6]=SButton(handle, sx+width+side, sy, width, height, 0, pBuf,  SELECT_BUTTON, 6, NULL); 
        gv.handle2[31]=Label(handle, sx, sy+height+side, width, height, "TURsensor",  clBlack);
        ...
        gv.handle2[33]=Label(handle, sx, sy+height+side+height+side+height+side+height+side, width, height, "COD_KHPsensor",  clBlack);
        ...
        gv.handle2[34]=Label(handle, sx, sy+height+side+height+side+height+side+height+side+height+side, width, height, "TURsensor",  clBlack);
    ...
    //TOC CALIBRATION SCREEN
        ...
        Label(handle, sx, sy+height+side, width, height, "TOCstd",  clBlack);
        gv.handle2[1]=SButton(handle, sx+width+side, sy+height+side, width, height, 0, pBuf,  NULL, 1, NULL); 
        ...
        Label(handle, sx, sy+height+side+height+side+height+side+height+side, width, height, "TOCreal",  clBlack);
        gv.handle2[3]=SButton(handle, sx+width+side, sy+height+side+height+side+height+side+height+side, width, height, "48G_Keyboard_Icon1.bmp", pBuf,  PUSH_BUTTON, 3, ULIKScreenEditButtonEvent); 
        
        Label(handle, sx, sy+height+side+height+side+height+side+height+side+height+side, width, height, "TOCstd_sensor",  clBlack);
        gv.handle2[4]=SButton(handle, sx+width+side, sy+height+side+height+side+height+side+height+side+height+side, width, height, 0, pBuf,  SELECT_BUTTON, 4, ULIKScreenEditButtonEvent); 
        ...
        sx = sx+width+width+side+side+side; 
        gv.handle2[21]=GroupPanel(handle, sx-5, sy-height, gwidth+side+gwidth+side, height+side+height+side+height+side+height+side, "1-point calibration value");
        gv.handle2[30]=Label(handle, sx, sy, width, height, "TOCstd_sensor",  clBlack);
        gv.handle2[6]=SButton(handle, sx+width+side, sy, width, height, 0, pBuf,  SELECT_BUTTON, 6, NULL); 
        gv.handle2[31]=Label(handle, sx, sy+height+side, width, height, "TURsensor",  clBlack);
        gv.handle2[7]=SButton(handle, sx+width+side, sy+height+side, width, height, 0, pBuf,  SELECT_BUTTON, 7, NULL); 
        gv.handle2[32]=Label(handle, sx, sy+height+side+height+side, width, height, "TOCreal",  clBlack);
        gv.handle2[8]=SButton(handle, sx+width+side, sy+height+side+height+side, width, height, 0, pBuf,  SELECT_BUTTON, 8, NULL); 
        
        gv.handle2[22]=GroupPanel(handle, sx-5, sy+height+side+height+side+height+side+height+side-height, gwidth+side+gwidth+side, height+side+height+side+height+side+height+side, "2-point calibration value");
        gv.handle2[33]=Label(handle, sx, sy+height+side+height+side+height+side+height+side, width, height, "TOCstd_sensor",  clBlack);
        gv.handle2[9]=SButton(handle, sx+width+side, sy+height+side+height+side+height+side+height+side, width, height, 0, pBuf,  SELECT_BUTTON, 9, ULIKScreenEditButtonEvent); 
        gv.handle2[34]=Label(handle, sx, sy+height+side+height+side+height+side+height+side+height+side, width, height, "TURsensor",  clBlack);
        gv.handle2[10]=SButton(handle, sx+width+side, sy+height+side+height+side+height+side+height+side+height+side, width, height, 0, pBuf,  SELECT_BUTTON, 10, ULIKScreenEditButtonEvent); 
        gv.handle2[35]=Label(handle, sx, sy+height+side+height+side+height+side+height+side+height+side+height+side, width, height, "CODreal",  clBlack);
    ...
    //TUR CALIBRATION SCREEN
        ...
        sx = sx+width+width+side+side+side; 
        gv.handle2[21]=GroupPanel(handle, sx-5, sy-height, gwidth+side+gwidth+side, height+side+height+side+height+side+height+side, "1-point calibration value");
        gv.handle2[30]=Label(handle, sx, sy, width, height, "TURorg",  clBlack);
        gv.handle2[6]=SButton(handle, sx+width+side, sy, width, height, 0, pBuf,  SELECT_BUTTON, 6, NULL); 
        gv.handle2[31]=Label(handle, sx, sy+height+side, width, height, "TURstd",  clBlack);
        gv.handle2[7]=SButton(handle, sx+width+side, sy+height+side, width, height, 0, pBuf,  SELECT_BUTTON, 7, NULL); 
    ...
    //COD_KPH CALIBRATION SCREEN
    else if(gv.ulikCalItem==3) {
        //좌측화면    
        GroupPanel(handle, sx-5, sy-height, gwidth+side+gwidth+side, height+side+height+side+height+side+height+side, "측정값");
        
        Label(handle, sx, sy, width, height, "COD_KHP",  clBlack);
        gv.handle2[0]=SButton(handle, sx+width+side, sy, width, height, 0, pBuf,  NULL, 0, NULL); 
        ...
        Label(handle, sx, sy+height+side+height+side+height+side+height+side, width, height, "COD_KHP",  clBlack);
        gv.handle2[3]=SButton(handle, sx+width+side, sy+height+side+height+side+height+side+height+side, width, height, "48G_Keyboard_Icon1.bmp", pBuf,  PUSH_BUTTON, 3, ULIKScreenEditButtonEvent); 
        ...
        sx = sx+width+width+side+side+side; 
        gv.handle2[21]=GroupPanel(handle, sx-5, sy-height, gwidth+side+gwidth+side, height+side+height+side+height+side+height+side, "1-point calibration value");
        gv.handle2[30]=Label(handle, sx, sy, width, height, "COD_KHPorg",  clBlack);
        gv.handle2[6]=SButton(handle, sx+width+side, sy, width, height, 0, pBuf,  SELECT_BUTTON, 6, NULL); 
        gv.handle2[31]=Label(handle, sx, sy+height+side, width, height, "COD_KHP",  clBlack);
        gv.handle2[7]=SButton(handle, sx+width+side, sy+height+side, width, height, 0, pBuf,  SELECT_BUTTON, 7, NULL); 
        gv.handle2[32]=Label(handle, sx, sy+height+side+height+side, width, height, "NONE",  clBlack);
        gv.handle2[8]=SButton(handle, sx+width+side, sy+height+side+height+side, width, height, 0, pBuf,  SELECT_BUTTON, 8, NULL); 
