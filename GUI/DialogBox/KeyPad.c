...
******************************************************************************/
volatile char gKeyPad_String[128];
...
//    char    buf[32];
	...
	if(win->ID == 15)
	{
		...
		else
        {
			...
			strcpy((char *)gKeyPad_String, (char *)gKeypadBuf);
            //Dprintf("KeyPad==1:: %s===%s\n", (char *)gKeypadBuf, (char *)gKeyPad_String);
        }
		first=0;
...
static int	 KeyPadCreateEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	...
	//Dprintf("KeyPadCreateEvent::Start\n");
...
//Dprintf("KeyPad::parent=%d handle=%d\n", parent, handle);   
//Dprintf("KeyPadPaintEvent::Z=%d\n", win->Zorder);    
	return(1);
}
...
//Dprintf("KeyPad::Start\n");
	
	strcpy((char *)gKeyPad_String, value);
