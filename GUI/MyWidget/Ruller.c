...
int RullerSetWidgetData(U32 id, void *Data, int cnt)
{	
...
int Ruller_TimerEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			page=0;
	int			sx=0, sy=0;
...
int Ruller_MouseOnEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			page=0;
	int			sx=0, sy=0;
...
int Ruller_MouseOffEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			page=0;
	int			sx=0, sy=0;
...
int Ruller_CreateEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	//U32		timerID=0;
	int			page=0;
	int			sx=0, sy=0;
...
//위젯에서는 PaintDrawControl()을 사용하면 안된다.
int Ruller_PaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
//	int			page=0;
	int			sx=0, sy=0;
	...
	*/
	//사용자 프로그램 영역
	{
		...
		Gr->FillRect(0, 0, win->width, win->height);	//바탕색을 칠한다.
		Gr->SetColor(clBlack);
		Gr->Rectangle(0, 0, win->width-1, win->height-1);	//테두리를 그린다.
		
		sx= win->width / 2;
		sy = 0;
		int  len = (win->width / 2);		
		Gr->SetColor(clBlack);		//눈금의 컬러
		for(; sy < win->height-1; sy+=60) 		Gr->HLine(sx, sy, len);

		sy = 0;
		len = len / 2;
		sx= win->width - (win->width / 4);
		for(; sy < win->height-1; sy+=30) 	Gr->HLine(sx, sy, len);

		sy = 0;
		len = len / 2;
		sx= win->width - (win->width / 8);
		for(; sy < win->height-1; sy+=6) 	Gr->HLine(sx, sy, len);
		
		SetFontAliasing("ng20_botong.sif");
		//sy = (win->height - 24) / 2;	
		
		// Right Allign, Max Value
		char	buf[64];
		sprintf(buf,"%5.1f", (float)win->Param);
		int text_width = TextLenAliasing(buf, "ng20_botong.sif");
		sx = (win->width - text_width) - 5;
		Gr->TextOutAliasing(sx, 0, buf, clBlack, 0, "ng20_botong.sif");		

		// Right Allign, Min Value
		text_width = TextLenAliasing("0.0", "ng20_botong.sif");
		sx = (win->width - text_width) - 5;
		Gr->TextOutAliasing(sx, win->height-20, "0.0", clBlack, 0, "ng20_botong.sif");		
		//
		//Gr->BitBltRop(SYSTEM_USER_BMP_WIDGET, 0, 0, 0, 100, 22, 26, ROP2, ROP2_AND);		
	}
	Gr->Draw(parent, handle, command, param, Data);	
	return(1);
}
...
U32 Ruller(HANDLE ContainerHandle,
			int sx, int sy, int width, int height,
			int backcolor, int max	)
{
	HANDLE		handle;
	U32			id;
	Windows		*parent_win;
	...
	handle=makeWindow(
				...
				Ruller_CreateEvent,		// CreateEvent
				Ruller_PaintEvent,			// PaintEvent
				Ruller_MouseOnEvent,	// MouseOnEvent
				Ruller_MouseOffEvent,	// MouseOffEvent
				NULL,											// MouseMoveEvent
				NULL,											// GestureEvent
				NULL,										// ClickEvent
				NULL,			// TimerEvent
				...
				max,												// PARAMETER, window->Param에 저장된다.
