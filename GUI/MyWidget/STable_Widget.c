...
WaterCell_Record_Data *WaterCell_Record;

//WaterCell_Record = (WaterCell_Record_Data *)USER_SCREEN12_ADDR;
/******************************************************************************
							Function Prototype
******************************************************************************/
...
int STable_RecordDisplay(Windows *win, Graphics  *Gr, int col_no,  int array_no, int select_display);
/******************************************************************************
							Example Program
******************************************************************************/
//============================================================================================================
...
int SetWidget_TableI_Scroll(U32 id,  int row_cnt, int mode);
...
//============================================================================================================
int	STable_TimerEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			page=0;
	int			sx=0, sy=0;
...
void STable_Cell_Display(WaterCell_Record_Data data, int index)
{
	switch(index)
	{
		case 0:  Dprintf("%s \n", data.Mode);  break;
		case 1:  Dprintf("%s \n", data.Date); break;
		case 2:  Dprintf("%6.1f \n", data.StackVolt); break;
		case 3:  Dprintf("%6.1f \n", data.StackCurrent); break;
		case 4:  Dprintf("%6.1f \n", data.ACVolt); break;
		case 5:  Dprintf("%6.1f \n", data.ACCurrent); break;
		case 6:  Dprintf("%6.1f \n", data.Temp); break;
		case 7:  Dprintf("%6.1f \n", data.pH); break;
		case 8:  Dprintf("%6.1f \n", data.DCVolt); break;
		case 9:  Dprintf("%1d \n", data.Level); break;
...
// 위젯에서는 PaintDrawControl()을 사용하면 안된다.
//============================================================================================================
int	STable_MouseOnEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
//	int			page=0;
//	int			sx=0, sy=0;
	int			line=0;
	...
	if(Current_Cursor_Y < TableData_Count) //빈공백을 가르키지 않으면 처리한다. 빈공백에 마캉되는것을 방지하기위함
	{
		...
		STable_RecordDisplay(win, Gr, line, current_line+line, YES);			
	}	
	//Dprintf("Table_MouseOnEvent:: CELL[%d, %d]=", Current_Cursor_X, Current_Cursor_Y );
	STable_Cell_Display(WaterCell_Record[Current_Cursor_Y], Current_Cursor_X);
...
// 위젯에서는 PaintDrawControl()을 사용하면 안된다.
//============================================================================================================
int	STable_MouseOffEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			page=0;
	int			sx=0, sy=0;
...
//============================================================================================================
int	STable_CreateEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
//	U32		timerID=0;
	int			page=0;
	int			sx=0, sy=0;
	...
	*/
	...
	switch(Display_Index) 
	{
		case 0: 
		{
			...
			{
				strcpy(sbuf, WaterCell_Record[array_no].Mode);
		...
		case 1: 
		{
			if(Cell_Data[cell]==STRING_TYPE)
			{
				strcpy(sbuf, WaterCell_Record[array_no].Date);
				RightTrim_String(sbuf);
				len = LowTextLen(sbuf, fArial12);	
				if(Cell_Align[cell] == LEFT_ALIGN) dx = 3;
				else if(Cell_Align[cell] == CENTER_ALIGN) dx =  ( Cell_Info[cell] - len) / 2;
		...
		case 2: 
		{
			...
			{
				sprintf(sbuf, "%6.1f", WaterCell_Record[array_no].StackVolt);
				RightTrim_String(sbuf);
				len = LowTextLen(sbuf, fArial12);	
				if(Cell_Align[cell] == LEFT_ALIGN) dx = 3;
				else if(Cell_Align[cell] == CENTER_ALIGN) dx =  ( Cell_Info[cell] - len) / 2;
		...
		case 3: 
		{
			if(Cell_Data[cell]==FLOAT_TYPE)
			{
				sprintf(sbuf, "%6.1f", WaterCell_Record[array_no].StackCurrent);
				RightTrim_String(sbuf);
				len = LowTextLen(sbuf, fArial12);	
				if(Cell_Align[cell] == LEFT_ALIGN) dx = 3;
				else if(Cell_Align[cell] == CENTER_ALIGN) dx =  ( Cell_Info[cell] - len) / 2;
		...
		case 4: 
		{
			if(Cell_Data[cell]==FLOAT_TYPE)
			{
				sprintf(sbuf, "%6.1f", WaterCell_Record[array_no].ACVolt);
				RightTrim_String(sbuf);
				len = LowTextLen(sbuf, fArial12);	
				if(Cell_Align[cell] == LEFT_ALIGN) dx = 3;
				else if(Cell_Align[cell] == CENTER_ALIGN) dx =  ( Cell_Info[cell] - len) / 2;
		...
		case 5: 
		{
			if(Cell_Data[cell]==FLOAT_TYPE)
			{
				sprintf(sbuf, "%5.1f", WaterCell_Record[array_no].ACCurrent);
				RightTrim_String(sbuf);
				len = LowTextLen(sbuf, fArial12);	
				if(Cell_Align[cell] == LEFT_ALIGN) dx = 3;
				else if(Cell_Align[cell] == CENTER_ALIGN) dx =  ( Cell_Info[cell] - len) / 2;
		...
		case 6: 
		{
			if(Cell_Data[cell]==FLOAT_TYPE)
			{
				sprintf(sbuf, "%5.1f", WaterCell_Record[array_no].Temp);
				RightTrim_String(sbuf);
				len = LowTextLen(sbuf, fArial12);	
				if(Cell_Align[cell] == LEFT_ALIGN) dx = 3;
				else if(Cell_Align[cell] == CENTER_ALIGN) dx =  ( Cell_Info[cell] - len) / 2;
		...
		case 7: 
		{
			if(Cell_Data[cell]==FLOAT_TYPE)
			{
				sprintf(sbuf, "%6.1f", WaterCell_Record[array_no].pH);
				RightTrim_String(sbuf);
				len = LowTextLen(sbuf, fArial12);	
				if(Cell_Align[cell] == LEFT_ALIGN) dx = 3;
				else if(Cell_Align[cell] == CENTER_ALIGN) dx =  ( Cell_Info[cell] - len) / 2;
		...
		case 8: 
		{
			if(Cell_Data[cell]==FLOAT_TYPE)
			{
				sprintf(sbuf, "%5.1f", WaterCell_Record[array_no].DCVolt);
				RightTrim_String(sbuf);
				len = LowTextLen(sbuf, fArial12);	
				if(Cell_Align[cell] == LEFT_ALIGN) dx = 3;
				else if(Cell_Align[cell] == CENTER_ALIGN) dx =  ( Cell_Info[cell] - len) / 2;
		...
		case 9: 
		{
			...
			{
				sprintf(sbuf, "%5d", WaterCell_Record[array_no].Level);
				RightTrim_String(sbuf);
				len = LowTextLen(sbuf, fArial12);	
				if(Cell_Align[cell] == LEFT_ALIGN) dx = 3;
				else if(Cell_Align[cell] == CENTER_ALIGN) dx =  ( Cell_Info[cell] - len) / 2;
...
// 위젯에서는 PaintDrawControl()을 사용하면 안된다.
//============================================================================================================
int	STable_PaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			page=0, i;
	int			sx=0, sy=0;
//	char			sbuf[CELL_MAX_STRING];
...
//	static 		int last_line=0;
	...
	else
	{
		...
		{
			if(WaterCell_Record != NULL) free(WaterCell_Record);			
			//SD에서 cnt만큼의 레코드를 읽어들여 WaterCell_Record[]에 저장한다.
			retcnt = SDtoRecord(STable_FileName, STable_Date, 60, 1);
			if(retcnt) //시간단위검색
			{
				/*
				//내용을 디버거로 출력한다.
				for(int i=0; i<retcnt; i++)
					DisplayStruct_Index(WaterCell_Record[i], Display_Index);	//UART Display	
			...
			{
				for(i=0; i<retcnt; i++)
					STable_RecordDisplay(win, Gr, i, i, NO);
				first_line = 0;
				...
				if((retcnt - current_line) < Cell_Colume_Cnt)
				{
					for(i=current_line; i<current_line+(retcnt - current_line); i++)
					{
						if(Current_Cursor_Y == i+current_line)
							STable_RecordDisplay(win, Gr, i-current_line, i, YES);			
						else
							STable_RecordDisplay(win, Gr, i-current_line, i, NO);			
					}
				}
				else
				{
		...
		else if(command==2) //UP command
		{
			if(current_line==0)
			{
				...
				if(retcnt < Cell_Colume_Cnt)
				{
					...
					{
						if(Current_Cursor_Y == i+current_line)
							STable_RecordDisplay(win, Gr, i, i, YES);
						else
							STable_RecordDisplay(win, Gr, i, i, NO);
					}
				}
...
int	 STabel_CloseEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle,"");;
...
U32 STable(HANDLE ContainerHandle, 				// 위젯윈도우이므로 컨테이너를지정해야한다.
...
//	Cell_Row_Cnt = row_cnt;
	...
	handle=makeWindow(
				...
				STable_CreateEvent,	// CreateEvent
				STable_PaintEvent,		// PaintEvent
				STable_MouseOnEvent,	// MouseOnEvent
				NULL,// MouseOffEvent
				NULL,					// MouseMoveEvent 가등록되면 선택된 셀이 표시된다.
				NULL,					// GestureEvent
				NULL,			// ClickEvent
				STable_TimerEvent,	// TimerEvent
				NULL,  				// ControlEvent
				NULL,					// ChangeEvent
                NULL,           		// KeyEvent(Button)
                NULL,                 	// EncoderEvent
                NULL,                 	// FocusEvent
                STabel_CloseEvent,		// CloseEvent
				NULL,					// PARAMETER
				NULL,					// Back Image
				NULL,					// 
				NULL,					// Windows Text
