...
int FontLabelClearFontOut(Windows *win, Graphics *Gr, char *string, int sx, int sy)
{
    int     FontX_pos;
...
int FontTextWid_Cal(char *str)
{
    int     cnt=0;
    int     wid = 0;
    while(*str)
    {
        cnt = FontSpace1[*str-0x20]+TEXT_INTERVAL;
        wid += cnt;
        str++;
    }
    return(wid);
...
int FontLabelMouseOnEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			sx=0, sy=0;
	...
	if(win->Text != NULL)
	{
        ...
        text_width = FontTextWid_Cal(win->Text);
		...
		sy = (win->height - 24) / 2; //33 은 폰트 Y Size
			
		if(sx < 0) sx = 0;
		if(sy < 0) sy = 0;

        FontLabelClearFontOut(win, Gr, win->Text, sx, sy/2);
	}
...
int FontLabelPaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			sx=0, sy=0;
	int			text_width, page=0;
			...
			)
{
	...
	parent_win = GetWindow(NULL, ContainerHandle, "FontLabel");
	...
	handle=makeWindow(
				...
				NULL,						// CreateEvent
				FontLabelPaintEvent,			// PaintEvent,LabelPaintEvent
				FontLabelMouseOnEvent,		// MouseOnEvent
				FontLabelPaintEvent,		// MouseOffEvent
				NULL,						// MouseMoveEvent
				NULL,						// GestureEvent
				NULL,						// ClickEvent
				NULL,						// TimerEvent
