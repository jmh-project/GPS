...
void SystemButtonDraw(int  press, Windows *win, Graphics *Gr, int sx, int sy)
{
	// ColorCode
...
int	SystemButtonMouseOnEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			sx=0, sy=0;
//	int			text_width=0;
	...
	Gr->FillRect(0, 0, 0, 0);
	//!< 버튼의 외각을그린다.
	SystemButtonDraw(ON, win, Gr, sx, sy);

	Gr->Draw(parent, handle, command, param, Data);	
	return(1);
}

int	SystemButtonMouseOffEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			sx=0, sy=0;
//	int			text_width;
	...
	Gr->FillRect(0, 0, 0, 0);	
	//!< 버튼의 외각을그린다.
	SystemButtonDraw(OFF, win, Gr, sx, sy);
...
int	SystemButtonPaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle," ");;
...
//	PARAM	parameter;
    ...
    if(parent_win==0)
    {
        Dprintf("SystemButton::Alloc Error\n");
		return(0);
    }
    ...
    handle=makeWindow(
				...
				NULL,						// CreateEvent
				SystemButtonPaintEvent,			// PaintEvent
				SystemButtonMouseOnEvent,			// MouseOnEvent
				SystemButtonMouseOffEvent,		// MouseOffEvent
				NULL,						// MouseMoveEvent
				NULL,						// GestureEvent
