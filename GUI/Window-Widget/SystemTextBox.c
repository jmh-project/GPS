...
int SystemTextBoxTextDraw(Windows *win, Graphics *Gr)
{
	int	sx, sy;
	int text_width;
	...
	if(strlen(win->Text))		
	{
		...
		else if(win->FontNo == 48)
		{
			...
			//Dprintf("%s:: 48Size=> text_wid=%d sx=%d sy=%d \n", __FILE__, text_width,  sx, sy);
			if(win->BackColor==0)
				Gr->TextOutAliasing(sx, sy, win->Text, win->FontColor, 0, "mg48_e_Bold.sif");
			else
				Gr->TextOutAliasing(sx, sy, win->Text, win->FontColor, 1, "mg48_e_Bold.sif");
		}
		else if(win->FontNo == 64)
		{
			SetFontAliasing("mg64_e_Bold.sif");
			text_width = TextLenAliasing(win->Text, "mg64_e_Bold.sif");
			//sx = (win->width - text_width) / 2;
			sy = (win->height - 64) / 2;
			if(win->Param==RIGHT_ALIGN)
			{
			...
			if(win->BackColor==0)
				...
				Gr->TextOutAliasing(sx, sy, win->Text, win->FontColor, 1, "mg64_e_Bold.sif");
		}
	}		
	return(1);
}
...
void SystemTextBoxDraw(int  press, Windows *win, Graphics *Gr, int sx, int sy)
{
	...
	TextBoxStyle_Draw(win, Gr,  sx, sy, width, height);
	if(press==ON)
	{
		Gr->SetColor(clRed);		
	...
	SystemTextBoxTextDraw(win, Gr);
}
		
int	SystemTextBoxMouseOnEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			sx=0, sy=0;
...
int	SystemTextBoxMouseOffEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	Graphics 	*Gr= GetGraphics(parent, handle, CLIENT_AREA);
	Windows 	*win = GetWindow(parent, handle,"");
	...
	if(win->Attr == READ_ONLY) return(0);	
 	//Dummy
	Gr->FillRect(0, 0, 0, 0);
	SystemTextBoxDraw(OFF, win, Gr, sx, sy);
...
int	SystemTextBoxPaintEvent(HANDLE parent, HANDLE handle, WCOMMAND command, PARAM param, void *Data)
{
	int			sx=0, sy=0;
...

{
	...
	handle=makeWindow(
				...
				NULL,						// CreateEvent
				SystemTextBoxPaintEvent,			// PaintEvent
				SystemTextBoxMouseOnEvent,		// MouseOnEvent
				SystemTextBoxMouseOffEvent,		// MouseOffEvent
				NULL,						// MouseMoveEvent
				NULL,						// GestureEvent
