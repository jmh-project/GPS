...
void USBH_MSC_HandleBOTXfer (USB_OTG_CORE_HANDLE *pdev ,USBH_HOST *phost)
{
  ...
  static uint32_t remainingdLength;
  ...
  {  
    ...
    case USBH_MSC_SENT_CBW:
      ...
      { 
        ...
        {
          remainingdLength = USBH_MSC_CBWData.field.CBWTransferLength ;
    ...
    case USBH_MSC_BOT_DATAIN_STATE:
      ...
      if((URB_Status == URB_DONE) ||(USBH_MSC_BOTXferParam.BOTStateBkp != USBH_MSC_BOT_DATAIN_STATE))
      {
        ...
        if(remainingdLength > MSC_Machine.MSBulkInEpSize)
        {
          ...
          remainingdLength -= MSC_Machine.MSBulkInEpSize;
          datapointer = datapointer + MSC_Machine.MSBulkInEpSize;
        }
        else if ( remainingdLength == 0)
        {
        ...
        {       
          ...
          remainingdLength = 0; /* Reset this value and keep in same state */
        }
      }
      else if(URB_Status == URB_STALL)
      {
    ...
    case USBH_MSC_BOT_DATAOUT_STATE:
      ...
      if(URB_Status == URB_DONE)
      {
        ...
        if(remainingdLength > MSC_Machine.MSBulkOutEpSize)
        {
          USBH_BulkSendData (pdev,
                             datapointer, 
          ...
          remainingdLength = remainingdLength - MSC_Machine.MSBulkOutEpSize;
        }
        else if ( remainingdLength == 0)
        {
          /* If value was 0, and successful transfer, then change the state */
