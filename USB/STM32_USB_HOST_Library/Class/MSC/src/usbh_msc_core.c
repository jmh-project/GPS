...
Dprintf("USBH_MSC Init\n");
        ...
        (pphost->device_prop.Itf_Desc[0].bInterfaceProtocol == MSC_PROTOCOL))
    {
        ...
        //강제로 설정되게하는경우이다.
        //이값은 스틱마다틀리다. 편법으로 사용할때 쓴다.
        /*        
        MSC_Machine.hc_num_out = 0x02;
        MSC_Machine.hc_num_in = 0x03;
        */
        if((MSC_Machine.hc_num_out==0xff||MSC_Machine.hc_num_in==0xff))
        {
            MSC_Machine.hc_num_out=0x03;
            MSC_Machine.hc_num_in=0x04; 
        }                
        Dprintf("MSC_Machine.hc_num_out=%04x MSC_Machine.hc_num_out=%04x\n", MSC_Machine.hc_num_out, MSC_Machine.hc_num_in); 
    ...
    {
        Dprintf("USBH_MSC_InterfaceInit::USB Device Not Support\n");
